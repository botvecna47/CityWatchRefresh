...
export function SupervisorManagement({ users, reports }: { users: User[], reports: Report[] }) {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!addForm.fullName || !addForm.email || !addForm.password || !addForm.areaName) {
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="fixed top-[10%] left-1/2 -translate-x-1/2 w-full max-w-md bg-white shadow-2xl z-50 rounded-2xl overflow-hidden border border-gray-100 flex flex-col p-6">
              ...
              <form onSubmit={handleAddSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input type="text" required value={addForm.fullName} onChange={e => setAddForm({ ...addForm, fullName: e.target.value })} className="w-full h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm font-medium focus:ring-2 focus:ring-[#1A4331]/20 outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Email</label>
                  <input type="email" required value={addForm.email} onChange={e => setAddForm({ ...addForm, email: e.target.value })} className="w-full h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm font-medium focus:ring-2 focus:ring-[#1A4331]/20 outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Password</label>
                  <input type="password" required minLength={6} value={addForm.password} onChange={e => setAddForm({ ...addForm, password: e.target.value })} className="w-full h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm font-medium focus:ring-2 focus:ring-[#1A4331]/20 outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Area Name</label>
                  {/* We can provide a dropdown of areas instead of input, but input is fine if we match it by name */}
                  <input type="text" required value={addForm.areaName} onChange={e => setAddForm({ ...addForm, areaName: e.target.value })} className="w-full h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm font-medium focus:ring-2 focus:ring-[#1A4331]/20 outline-none" placeholder="e.g. Anand Nagar" />
                </div>
                <div className="pt-4 flex gap-3">
                  <Button type="button" variant="outline" className="flex-1" onClick={() => setShowAddModal(false)}>Cancel</Button>
                  <Button type="submit" disabled={isAdding} className="flex-1 bg-[#1A4331] text-white hover:bg-[#2E7D32]">
